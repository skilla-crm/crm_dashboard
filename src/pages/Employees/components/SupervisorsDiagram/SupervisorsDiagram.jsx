...
const SupervisorsDiagram = ({ data, series }) => {
    const seriesLabels = Object.fromEntries(series.map((item) => [item.key, item.name]));
    ...
    return (
        <div className={s.root}>
            ...
            > */}
            <BarChart
                style={{ width: '100%', maxHeight: '336px', aspectRatio: 1.618, outline: 'none' }}
                responsive
                data={data?.map(el => { return { name: el.date, indicator: el.indicator } })}
            >
                <Bar
                    dataKey="indicator"
                    fill="#80DEF1"
