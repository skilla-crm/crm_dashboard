...
export const getLastDay = () => {
    ...
    const formatedLastDay =
        lastDay.split(' ')[0].length < 2 ? '0' + lastDay : lastDay;
...
export const getTitleDateDuration = (
    ...
    const lastDayMonth = dayjs(dateStart).endOf('month').format('D');
    ...
    if (
        ...
        return `${dayjs(dateStart).format('D MMMM')} — ${dayjs(dateEnd).format(
            'D MMMM'
        )}`;
    }
    ...
    if (startYear !== endYear) {
        return `${dayjs(dateStart).format('D MMMM YYYY')} — ${dayjs(
            dateEnd
        ).format('D MMMM YYYY')}`;
    }
};
